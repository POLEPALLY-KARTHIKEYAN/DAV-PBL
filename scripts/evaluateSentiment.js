...
// Simple JS version of the detector used in the app (kept lightweight for Node script)
...
for (const line of lines) {
  // Split at last comma to preserve commas in comment text
  ...
  if (pred === label) correct++;
    ...
    if (mismatches.length < 10) mismatches.push({ comment: comment.slice(0, 200), label, pred, confidence: detected ? detected.confidence : 0 });
  }
}
